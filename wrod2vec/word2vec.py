...
class CBOW:
    ...
    def train_one_step(self, context_tokens: List[str], target_token: str, learning_rate: float) -> float:
        ...
        # average of one hot vectors of `context_tokens`     
        x_bar = (sum([one_hot(len(self.vocab), self.vocab.token_to_idx(item)) for item in context_tokens])).reshape(-1, 1) # (V,1)
        # ground true
        t = one_hot(len(self.vocab), self.vocab.token_to_idx(target_token)).reshape(-1, 1) # (V,1)
        
        # ==== TODO: Forward step ====
        h = (self.W1.transpose() @ x_bar)/len(context_tokens) # (N,1)
        u = self.W2.transpose() @ h # (V,1)
        
        # ==== TODO: Calculate loss ====
        # prediction
        y = softmax(u) # (V,1)
        loss = -np.log(softmax(u))[t==1].squeeze() #(V,1)

        # ==== TODO: Update parameters ====
        e = y - t
        eh = self.W2 @ e # (N,1)
        #update
        self.W2 = self.W2 - learning_rate * (h @ e.transpose())
        self.W1 = self.W1 - learning_rate / len(context_tokens) * (x_bar @ eh.transpose())
